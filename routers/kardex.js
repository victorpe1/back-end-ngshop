...
router.get(`/kardex_simple/:id`, async (req, res) => {
  ...
  for (var venta of ventaListaFiltro) {
    ...
    var data = {
      ...
      valor_total_venta:
        venta.order_prods[0].producto.precio * venta.order_prods[0].cantidad,
  ...
  aux = aux.sort((a, b) => {
    if (a.fecha_create < b.fecha_create) return -1;
    if (a.fecha_create > b.fecha_create) return 1;
    return 0;
  });

  let stock_act = 0;
  let stock_actual = await Producto.findById(req.params.id).select(
    "cont_stock"
  );
  ...
  for (let i = 1; i < aux.length; i++) {
    if (aux[i].id_compra == "") {
      ...
      var data = {
        ...
        precio_existencia:
          Math.round(
            ((valor_anterior - aux[i].valor_total_venta) /
              (cantidad_anterior - aux[i].cantidad_venta)) *
              100
          ) / 100,
      ...
      var data = {
        ...
        precio_existencia:
          Math.round(
            ((valor_anterior + aux[i].valor_total_compra) /
              (cantidad_anterior + aux[i].cantidad_compra)) *
              100
          ) / 100,
...
async function callMeMayBe( idProducto) {
  ...
  const compraListaFiltro = compraLista.filter(
    (compra) => compra.producto.id == idProducto
  );
  ...
  const ventaListaFiltro = pedido.filter(
    (venta) => venta.order_prods[0].producto._id == idProducto
  );
  ...
  let stock_act = 0;
  let stock_actual = await Producto.findById(idProducto).select(
    "cont_stock"
  );
  stock_act = stock_actual.cont_stock;
  ...
  json_existencia = json_existencia.sort((a, b) => {
    if (a.fecha_create > b.fecha_create) return -1;
    if (a.fecha_create < b.fecha_create) return 1;
    return 0;
  });
  ...
  for (let i = 1; i < json_existencia.length; i++) {
    var data = {
    }
  }

  
  return json_existencia;
}
